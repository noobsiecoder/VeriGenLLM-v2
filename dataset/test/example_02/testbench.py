...
Checks verilog code if it has `assign ..` statement
It also verifies if it only has one statement
...
def check_code(solution_code: str) -> bool:
    ...
    for line in solution_code.strip().split("\n"):
        ...
        if match:
            occurence += 1
            log.info(
                f"Wire assigned in solution code: {match.group(1)} and occurence = {occurence}"
            )
